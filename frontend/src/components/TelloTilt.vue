<template>
  <div class="layout-padding logo-container non-selectable no-pointer-events">
    <div id="tello-front" class="logo" :style="position">
      <img width="100%" height="100%" src="~assets/tello-front.png">
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    move (evt) {
      const logoElement = document.getElementById('tello-front')
      let { height, width } = dom
      height = height(logoElement)
      width = width(logoElement)
      const
